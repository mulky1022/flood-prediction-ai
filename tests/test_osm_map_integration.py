...
def test_humanitarian_osm_tile_request():
    ...
    try:
        r = httpx.get(hot_url, headers={"User-Agent": "SriLanka-FloodWatch/1.0"}, timeout=10)
        assert r.status_code == 200
        assert r.headers.get("content-type") == "image/png"
        assert len(r.content) > 1000
    except httpx.RequestError:
        pytest.skip("External HOT tile network request unavailable")
...
def test_osm_standard_tile_request():
    ...
    try:
        r = httpx.get(osm_url, headers={"User-Agent": "SriLanka-FloodWatch/1.0"}, timeout=10)
        assert r.status_code == 200
        assert r.headers.get("content-type") == "image/png"
        assert len(r.content) > 1000
    except httpx.RequestError:
        pytest.skip("External OSM tile network request unavailable")
...
def test_all_33_monitoring_stations_api():
    ...
    for loc in locations:
        assert 5.5 <= float(loc["latitude"]) <= 10.0, f"Latitude {loc['latitude']} out of bounds for {loc['place_name']}"
        assert 79.0 <= float(loc["longitude"]) <= 82.5, f"Longitude {loc['longitude']} out of bounds for {loc['place_name']}"
