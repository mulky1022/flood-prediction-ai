...
HEADERS = {"User-Agent": "SriLankaFloodWatch-VerificationSuite/1.0"}
...
def test_api_locations_total_and_schema():
    req = urllib.request.Request(f"{BASE_URL}/locations", headers=HEADERS)
...
def test_kolonnawa_coordinates():
    req = urllib.request.Request(f"{BASE_URL}/locations/1", headers=HEADERS)
    with urllib.request.urlopen(req, timeout=10) as resp:
        assert resp.status == 200
...
def test_prediction_endpoint_integration():
    req = urllib.request.Request(f"{BASE_URL}/predict/1", headers=HEADERS)
