...
async function resolveAndLoadLocation() {
  ...
  // PRIORITY 1: Explicit URL parameter exists
  if (paramId && !isNaN(parseInt(paramId, 10))) {
    const id = parseInt(paramId, 10);
    await loadStationDetails(id, {
      type: 'url',
      message: 'Viewing station specified in URL parameter.'
    });
  ...
  // PRIORITY 2 & 3: No location_id in URL → Request browser geolocation / IP fallback
...
function updateUrlLocation(locationId) {
  try {
    ...
    window.history.replaceState({ location_id: locationId }, '', url.toString());
  } catch (e) {}
}
...
async function loadStationDetails(locationId, contextInfo = {}) {
  ...
  try {
    const [locResult, weatherResult, predResult, historyResult, alertsResult] = await Promise.allSettled([
      api.getLocation(locationId),
      api.getWeather(locationId),
      api.getPrediction(locationId),
      api.getPredictionHistory(locationId, 10),
      api.getAlertsByLocation(locationId)
    ]);

    if (thisRequestId !== currentRequestId) return;

    // 1. Process Location Details
    if (locResult.status === 'fulfilled' && locResult.value) {
      stationData = locResult.value;
      renderBreadcrumbs(stationData);
      renderHeaderBanner(stationData, []);
      renderVulnerabilityMatrix(stationData);
    } else {
      stationData = null;
      renderLocationError('Unable to load monitoring locations.');
    }

    // 2. Process Alerts
    if (alertsResult.status === 'fulfilled' && alertsResult.value) {
      const allAlerts = alertsResult.value.items || [];
      activeAlertsData = allAlerts.filter((a) => a.status === 'ACTIVE' || a.status === 'ACKNOWLEDGED');
      renderActiveAlerts(activeAlertsData);
    } else {
      activeAlertsData = [];
      renderAlertsError('Alert information is temporarily unavailable.');
    }

    // 3. Process Weather Telemetry
    if (weatherResult.status === 'fulfilled' && weatherResult.value) {
      ...
      weatherData = null;
      renderWeatherError('Weather data is temporarily unavailable.');
    }

    // 4. Process ML Prediction
    if (predResult.status === 'fulfilled' && predResult.value) {
      predictionData = predResult.value;
      renderPredictionEngine(predictionData, stationData, activeAlertsData);
      renderDecisionThreshold(predictionData);
      renderLiveCatchmentAnalysis(predictionData, stationData, weatherData);
    } else {
      predictionData = null;
      renderPredictionError('Flood prediction is temporarily unavailable.');
    }

    // 5. Process Prediction History
    if (historyResult.status === 'fulfilled' && historyResult.value) {
      historyData = historyResult.value.items || [];
