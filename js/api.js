...
export const api = {
  ...
  /**
   * Get aggregated location details, canonical prediction, active alerts, and history summary
   * @param {number|string} locationId
   * @returns {Promise<{status: string, location: Object, current_prediction: Object|null, active_alerts: Array, recent_history: Array, status_flag: string}>}
   */
  async getLocationDetails(locationId) {
    return fetchJson(`/locations/${locationId}/details`);
  },

  /**
   * Get aggregated emergency flood risk & official warning payload (< 2KB)
   * @param {string} locationId
   * @param {string} [lang='en']
   * @returns {Promise<Object>}
   */
  async getEmergencyData(locationId, lang = 'en') {
    return fetchJson(`/emergency/${locationId}?lang=${lang}`, { timeout: 5000 });
  },
  ...
  /**
   * Get unified canonical current prediction for a location
   * @param {number|string} locationId
   * @returns {Promise<Object>}
   */
  async getCurrentPrediction(locationId) {
    return fetchJson(`/predictions/current/${locationId}`);
  },
  ...
  /**
   * Get predictions for all map stations
   * @returns {Promise<{status: string, total: number, predictions: Array}>}
   */
  async getMapPredictions() {
    return fetchJson('/predictions/map');
  },

  /**
   * Get prediction history for a specific location
   * @param {number|string} locationId
   * @param {Object|number} options - Query options { risk_level, start_date, end_date, limit, offset } or limit number
   * @param {number} offset
   * @returns {Promise<{status: string, location_id: number, total: number, items: Array}>}
   */
  async getPredictionHistory(locationId, options = 50, offset = 0) {
    ...
    if (typeof options === 'object' && options !== null) {
      ...
      query.set('limit', options || 50);
      query.set('offset', offset || 0);
    }
    const qs = query.toString();
    return fetchJson(`/predictions/history/${locationId}${qs ? '?' + qs : ''}`);
  },
  ...
  /**
   * Get prediction record by primary key prediction ID
   * @param {number|string} predictionId
   * @returns {Promise<Object>}
   */
  async getPredictionById(predictionId) {
    return fetchJson(`/predictions/id/${predictionId}`);
  },
  ...
  /**
   * Get current active alert for a specific location
   * @param {number|string} locationId
   * @returns {Promise<{status: string, alert: Object|null, message: string}>}
   */
  async getCurrentAlert(locationId) {
    return fetchJson(`/alerts/current/${locationId}`);
  },
  ...
  /**
   * Get current active official government warning for a location
   * @param {number|string} locationId
   * @returns {Promise<{status: string, state: string, location_id: number, warning: Object|null, message: string}>}
   */
  async getOfficialWarning(locationId) {
    return fetchJson(`/warnings/current/${locationId}`);
  },

  /**
   * Get historical official government warnings log for a location
   * @param {number|string} locationId
   * @returns {Promise<{status: string, total: number, location_id: number, warnings: Array}>}
   */
  async getOfficialWarningHistory(locationId) {
    return fetchJson(`/warnings/location/${locationId}`);
  }
};
