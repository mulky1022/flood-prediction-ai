...
async function loadAlertsAndHistory() {
  try {
    ...
    // 2. Fetch predictions in single batch call for all monitored stations
    let mapPredMap = new Map();
    try {
      const mapPredsRes = await api.getMapPredictions();
      if (mapPredsRes && mapPredsRes.predictions) {
        mapPredsRes.predictions.forEach(p => {
          mapPredMap.set(p.location_id, p);
        });
      }
    } catch (e) {
      console.warn('[Alerts] Map predictions batch request error:', e);
    }

    const entryPromises = allLocations.map(async (loc) => {
      try {
        const weatherData = await api.getWeather(loc.id).catch(() => null);
        const mapPred = mapPredMap.get(loc.id);

        const pred = mapPred ? {
          class: mapPred.risk_level === 'HIGH' || mapPred.risk_level === 'CRITICAL' ? 1 : 0,
          flood_probability: mapPred.flood_probability,
          flood_probability_percent: mapPred.flood_probability * 100,
          risk_level: mapPred.risk_level,
          prediction_id: mapPred.prediction_id
        } : null;
...
function renderRealAlertCard(alert) {
  ...
  const actionText = alert.action_message || alert.recommendation || 'Maintain routine monitoring.';
  const predIdText = alert.prediction_id ? `Pred ID: ${alert.prediction_id}` : `Loc ID: ${alert.location_id}`;
      ...
      <div class="flex flex-col gap-2 pl-2">
        ...
        <div class="bg-surface-container-low p-2.5 rounded-lg flex flex-col gap-1.5">
          ...
          <div class="text-xs text-on-surface bg-surface-container/60 p-1.5 rounded flex items-start gap-1">
            <span class="material-symbols-outlined text-secondary text-[14px] mt-0.5">warning</span>
            <span><strong>Canonical Action (${alert.action_code || 'MONITOR'}):</strong> ${actionText}</span>
          </div>
          ${alert.expires_at ? `
            <div class="text-[11px] text-on-surface-variant flex items-center gap-1 mt-0.5">
              <span class="material-symbols-outlined text-[13px]">schedule</span>
              <span>Valid until: ${common.formatTime(alert.expires_at)}</span>
            </div>
          ` : ''}
        </div>
      </div>
...
function renderHistoryTable(entries) {
  ...
  tbody.innerHTML = entries.map(item => {
    ...
    const predIdText = pred?.prediction_id ? `ID: ${pred.prediction_id}` : `Loc: ${loc.id}`;
    const modelVer = pred?.model_version ? `v${pred.model_version}` : 'v1.0.0';
      ...
      <tr class="hover:bg-surface-container-low/60 transition-colors">
        ...
        <td class="py-3 px-4">
          <div class="flex flex-col">
            ...
            <span class="font-label-md text-label-md text-on-surface-variant">${loc.district} Catchment • <span class="text-secondary font-mono">${predIdText}</span></span>
          </div>
        </td>
        ...
        <td class="py-3 px-4 font-data-timestamp text-data-timestamp text-on-surface-variant">
          RandomForest ${modelVer} / 64 feats
        </td>
        <td class="py-3 px-4">
